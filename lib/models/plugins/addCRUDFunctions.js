...
const sortDirectionToOperator = (
  ...
  const includeOperator = includeBeforeAfter ? 'e' : '';

  if (paginationDirection === FORWARDS) {
    switch (direction) {
      case 1:
      case 'asc':
        return `$gt${includeOperator}`;
      case -1:
      case 'desc':
        return `$lt${includeOperator}`;
      default:
        return null;
    }
...
const paginationToFilter = ({
  ...
  const sortConditions = _.reduce(
    ...
    ({ oldKeys, conditions }, direction, key) => {
      const operator = sortDirectionToOperator(direction, paginationDirection, includeBeforeAfter);
...
// TODO: this is now wont work corretly if includeBeforeAfter is true :(
...
async function getConnection({
  ...
  if ((afterId || beforeId) && !(before || after)) {
    const model = await this.findOne({
      _id: afterId || beforeId,
      ...parsedScopeFilter
    });
    if (afterId) {
      after = modelToCursor(model, sort);
    } else if (beforeId) {
      before = modelToCursor(model, sort);
    }
  }
